<?php
    function create_cropped_thumbnail($temp, $dest) {
        ...
        if ($_FILES['image']['error'] === 0) {
            if ($_FILES['image']['size'] <= $max_sixe) {
                ...
                if (!$message) {
                    ...
                    $thumbpath = 'uploads/cropped/' . $filename . '.' . $extention;
                    $moved = move_uploaded_file($_FILES['image']['tmp_name'], $path);
                    $thumb = create_cropped_thumbnail($path, $thumbpath);
                    $message = '<img src="' . $thumbpath .  '" alt="Foto">';
                }
            }
        }
    }
